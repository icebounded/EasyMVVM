...
class NetworkManager private constructor() {
    ...
    companion object {
        val INSTANCE: NetworkManager by lazy(LazyThreadSafetyMode.SYNCHRONIZED) {
            NetworkManager()
        }
    }
    ...
    fun init(context: Context) {
        ...
        if (isHigherThenLollipop()) {
            ...
            netType = if (isHigherThenM()) {
                kotlin.runCatching {
                    manager.getNetworkCapabilities(manager.activeNetwork)?.let { getNetType21(it) }
                }.getOrNull() ?: NetType.NONE
            } else {
                getNetType(manager)
            }
        } else {
            networkReceiver = NetworkReceiver(mContext, ::observerIsConnection, ::observerNetWorkType)
        }
    }
